...
///////
  ...
  void _addPlayers(Worksheet sheet, TrainEntity train) {
    ...
    for (int i = startIndex, pi = 0; pi < playersCount; i++, pi++) {
      ...
      range.setNumber(StatsCalculator.getDistance(player));
      range.cellStyle = defaultCellStyle;
      ...
      range.setText(
          StatsCalculator.calculateAvgPulse(player).toStringAsFixed(1));
      range.cellStyle = defaultCellStyle;

      // Макс скорость
      range = sheet.getRangeByIndex(i, 19);
      range.setText(
          StatsCalculator.calculateMaxPulse(player).toStringAsFixed(1));
      range.cellStyle = defaultCellStyle;
  ...
  void _addSplits(Worksheet sheet, TrainEntity train) {
    var offset = startIndex + train.players.length;
    final playersCount = train.players.length;

    final defaultCellStyle = workbook.styles.add('cell2');
    defaultCellStyle.fontSize = 11;
    defaultCellStyle.fontName = 'Arial Narrow';
    defaultCellStyle.hAlign = HAlignType.center;
    defaultCellStyle.vAlign = VAlignType.center;
    ...
    for (final split in train.exerciseSplits) {
      for (int i = offset, pi = 0; pi < playersCount; i++, pi++) {
        final player = train.players[pi];
        var range = sheet.getRangeByIndex(i, 1);
        range.setNumber(player.number.toDouble());
        range.cellStyle = defaultCellStyle;
        ...
        // Время
        final splitDuration = train.startTime.add(split.endFragment!).difference(train.startTime.add(split.startFragment!));
        range = sheet.getRangeByIndex(i, 4);
        range.cellStyle = defaultCellStyle;
        range.setText(
            '${splitDuration.inHours.remainder(60).toString().padLeft(2, '0')}:${splitDuration.inMinutes.remainder(60).toString().padLeft(2, '0')}:${splitDuration.inSeconds.remainder(60).toString().padLeft(2, '0')}');

        // Дистанция
        range = sheet.getRangeByIndex(i, 5);
        range.setNumber(StatsCalculator.getDistance(player, split.startFragment, split.endFragment));
        range.cellStyle = defaultCellStyle;
        ...
        // Максимальный пульс
        range = sheet.getRangeByIndex(i, 8);
        range.setNumber(StatsCalculator.calculateMaxPulse(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;

        // Средний пульс
        range = sheet.getRangeByIndex(i, 9);
        range.setNumber(StatsCalculator.calculateAvgPulse(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;

        // VO2max
        range = sheet.getRangeByIndex(i, 10);
        range.setNumber(StatsCalculator.calculatedVo2max(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;

        // Интенсивность
        range = sheet.getRangeByIndex(i, 11);
        range.cellStyle = defaultCellStyle;
        range.setNumber(StatsCalculator.calculatedIntensity(player, split.startFragment, split.endFragment) / 100.0);
        range.numberFormat = '0%';

        // Зоны ЧСС
        final hrStats = StatsCalculator.getHrStats(player, split.startFragment, split.endFragment);
        range = sheet.getRangeByIndex(i, 12);
        range.cellStyle = defaultCellStyle;
        range.setNumber(hrStats[2]);
        range.numberFormat = '0%';
        ...
        // Всего
        range = sheet.getRangeByIndex(i, 15);
        range.setNumber(StatsCalculator.getCalories(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;

        // Жиры
        range = sheet.getRangeByIndex(i, 16);
        range.setNumber(StatsCalculator.getFats(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;

        // Углеводы
        range = sheet.getRangeByIndex(i, 17);
        range.setNumber(StatsCalculator.getProteins(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;

        // Средняя скорость
        range = sheet.getRangeByIndex(i, 18);
        range.setText(
            StatsCalculator.calculateAvgPulse(player, split.startFragment, split.endFragment).toStringAsFixed(1));
        range.cellStyle = defaultCellStyle;

        // Макс скорость
        range = sheet.getRangeByIndex(i, 19);
        range.setText(
            StatsCalculator.calculateMaxPulse(player, split.startFragment, split.endFragment).toStringAsFixed(1));
        range.cellStyle = defaultCellStyle;
        ...
        // Trimp
        range = sheet.getRangeByIndex(i, 22);
        range.setNumber(StatsCalculator.getTrimp(player, split.startFragment, split.endFragment).toDouble());
        range.cellStyle = defaultCellStyle;
        range.cellStyle.borders.right.lineStyle = LineStyle.medium;

      }
      final splitStartIndex = offset;
      offset += playersCount - 1;

      // Фрагмент тренировки
      sheet.getRangeByName('C$splitStartIndex:C$offset').merge();
      sheet.getRangeByName('C$splitStartIndex:C$offset').setText(split.name);
      sheet.getRangeByName('C$splitStartIndex:C$offset').cellStyle = defaultCellStyle;

      // Трехцветная шкала интенсивности
      var conditionalFormats =
          sheet.getRangeByName('K$splitStartIndex:K$offset').conditionalFormats;
      var condition = conditionalFormats.addCondition();
      condition.formatType = ExcelCFType.colorScale;
      var colorScale = condition.colorScale!;
      colorScale.setConditionCount(3);
      ...
      // Трехцветная шкала 90 100
      conditionalFormats =
          sheet.getRangeByName('N$splitStartIndex:N$offset').conditionalFormats;
      condition = conditionalFormats.addCondition();
      condition.formatType = ExcelCFType.colorScale;
      colorScale = condition.colorScale!;
      colorScale.setConditionCount(3);
      ...
      // Трехцветная шкала Trimp
      conditionalFormats =
          sheet.getRangeByName('V$splitStartIndex:V$offset').conditionalFormats;
      condition = conditionalFormats.addCondition();
      condition.formatType = ExcelCFType.colorScale;
      colorScale = condition.colorScale!;
      colorScale.setConditionCount(3);
